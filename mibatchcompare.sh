...
cat "/tmp/subdirs.txt" | while read subdir; do
	...
	if [ ! -e "$reportcsv" ]; then
	echo "filename,general_format,general_format_version,general_encoded_application,video_format,video_format_version,video_codecid,video_width,video_height,video_displayaspectratio,video_framerate,video_colorspace,video_bitdepth,video_compression_mode,audio_format,audio_codecid,audio_channels,audio_samplingrate,audio_bitdepth general_format_profile,general_filesize, general_duration, video_displayaspectratio_string, video_standard" > "$reportcsv"
	fi
	...
	cat "${filemanifest}" | while read file; do
		...
		general_filesize=`mediainfo --Inform="File;%Size%" "${file}"`
		general_duration=`mediainfo --Inform="Duration;%" "${file}"`
		video_displayaspectratio_string=`mediainfo --Inform="Display;%Aspect_Ratio%" "${file}"`
		video_standard=`mediainfo --Inform="Standard;%" "${file}"`
